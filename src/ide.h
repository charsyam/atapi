...
#define HDIO_REQ        HDIO_GETGEO
#define HDIO_DRIVE_CMD      0x31f
...
class Ide : public Drive<Ide> {
public:
    Ide(const char *_device) : device_(_device) {
    }

    uint8_t *getATAPIString(uint8_t *buf, int size, uint8_t *result) {
        int fixed = 0;
        for (int i = 0; i < size; i += 2) {
            if (buf[i] != 0x20 && buf[i+1] != 0x20) {
                result[i+1-fixed] = buf[i];
                result[i-fixed] = buf[i+1];
            } else {
                fixed += 2;
            }
        }
    ...
    DriveInfo getDriveInfo() {
        ...
        if (fd != -1) {
            uint8_t buf[1024] = { 0xEC, 0, 0, 1, 0 };
            int ret = ::ioctl(fd, HDIO_DRIVE_CMD, buf);
            if (ret == 0) {
                PIDENTIFY_DEVICE_DATA data = (PIDENTIFY_DEVICE_DATA)(buf+4);
                strcpy(driveInfo.name, device_.c_str());
                getATAPIString(data->SerialNumber, 20, (uint8_t*)driveInfo.serial);
                getATAPIString(data->ModelNumber, 40, (uint8_t*)driveInfo.model);
                getATAPIString(data->FirmwareRevision, 8, (uint8_t*)driveInfo.firmware);
                driveInfo.sectors = data->Max48BitLBA[1] << 32 | data->Max48BitLBA[0];
            }
        }
